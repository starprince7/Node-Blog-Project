...
router.post('/create', async (req, res) => {
    ...
    try{
        ...
        blog ? res.status(200).redirect('/') : res.status(400).send('error creating post!')
    }
    catch(err) {
        console.log(err)
    }
...
router.get('/blog', async (req, res) => {
    const fakeBlog = {
        ...
        comments: [
            { author: 'rex-T', body: 'Its rex-T in the building!' }
        ]
    }
    try {
...
router.delete('/comments/:postId/:commentId', async (req, res)=> {
    ...
    try{
        const post = await Blog.findByIdAndUpdate(postId, { $pull: { comments: {_id: commentId} } }, {new: true})
        console.log(post)
